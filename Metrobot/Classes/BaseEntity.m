...
- (NSDictionary*)debugDictionaryValue {
    ...
    for (int i = 0; i < count; i++) {
        ...
        //NSString *key = [NSString stringWithFormat:@"%s %s", property_getName(properties[i]), property_getAttributes(properties[i])];
        NSString *key = [NSString stringWithUTF8String:property_getName(properties[i])];
        id value = [self valueForKey:key];
        
        NSString *propString = [NSString stringWithUTF8String:property_getAttributes(properties[i])];
        NSArray *attrArray = [propString componentsSeparatedByString:@","];
        NSString* typedKey = [NSString stringWithFormat:@"(%@) %@", attrArray[0], key];
        typedKey = [typedKey stringByReplacingOccurrencesOfString:@"\"" withString:@""];
        typedKey = [typedKey stringByReplacingOccurrencesOfString:@"T@" withString:@""];
        
        if (value) {
            
            if ([value isKindOfClass:[BaseEntity class]]) {
                [dict setObject:[value dictionaryValue] forKey:typedKey];
            }
            else if ([value isKindOfClass:[NSArray class]]) {
                [dict setObject:[self arrayByEliminatingBaseEntitiesInArray:value] forKey:typedKey];
            }
            else {
                [dict setObject:value forKey:typedKey];
            }
        }
        else {
            [dict setObject:@"(null)" forKey:typedKey];
        }
    }
...
- (NSString*)debugDescription {
    ...
    NSString* selfAsString = self.debugDictionaryValue.description;
    return [NSString stringWithFormat:@"%@ : %@", [super description], selfAsString];
}
